...
class LocalAStarPlanner(StableLocalAStarPlanner):
    ...
    def _planner_now(state: PlannerState) -> float:
        """Use simulation stamp when available so offline runs obey planner timers."""
        try:
    ...
    def plan_candidate(self, state: PlannerState, local_goal: Vec3, obstacles) -> Optional[CandidateTrajectory]:
        ...
        if self._bypass_active and abs(self._bypass_side) > 0.5:
            ...
            self._side_latch_until = max(self._side_latch_until, now + 0.25)
        else:
        ...
        # window instead of resampling a different subgoal every frame.  Keep this
        # short in the offline local-A* smoke tests, otherwise a bypass path can
        # keep pulling the vehicle sideways after the obstacle has been cleared.
        ...
        if replan_reason or not committed_valid:
            ...
            if selected is None:
                ...
                self.diagnostics = self._diag(
                    ...
                    front_blocked, corridor_blocked, self._bypass_diag(bypass_mode, target_body, now)
                )
                self.diagnostics.update(self._state_diag(stuck, committed_valid, committed_clear, now))
                return None
    ...
    def _track_body_path(self, state: PlannerState, local_goal: Vec3, local_map, body_path, nearest: float,
        ...
        metadata: Dict = {
            ...
            "rejoin_forced_left_sec": max(0.0, self._rejoin_forced_until - now),
        ...
        extra = self._bypass_diag(bypass_mode, target_body, now)
        extra.update(
            {
                "planner_state": self._planner_state,
                "label": self._committed_label,
        ...
        extra.update(self._state_diag(self._last_stuck, True, clear, now))
    ...
    def _direct_bypass_candidate(self, state: PlannerState, local_goal: Vec3, local_map, nearest: float,
        ...
        if clear < max(0.45, self.config.hard_clearance + 0.12):
            return None
        self._committed_label = f"direct_bypass_{'left' if side > 0 else 'right'}"
        self._committed_until = now + 0.45
    ...
    def _bypass_target(self, points, mission_target_body: Tuple[float, float], now: float,
        ...
        if self._bypass_active:
            min_hold = now - self._bypass_started >= 0.45
            clear_to_goal = self._target_corridor_clear(points, mission_target_body)
            goal_behind_or_side = mission_target_body[0] < 1.2
            side_error_large = abs(mission_target_body[1]) > 1.2 and not mission_corridor_blocked
            timed_out = now - self._bypass_started > 1.6
            ...
            self._bypass_until = max(self._bypass_until, now + 0.20)
    ...
    def _clear_bypass(self, now: float, force_rejoin: bool) -> None:
        ...
        self.tracker.reset()
        if force_rejoin:
            self._rejoin_forced_until = now + 0.6

    def _make_bypass_goal(self, side: float) -> Tuple[float, float]:
        bx = min(2.4, max(1.6, 0.45 * self.config.astar_local_goal_dist))
        by = float(side) * max(1.05, self.config.drone_radius + self.config.obstacle_margin + 0.55)
    ...
    def _state_diag(self, stuck: bool, committed_valid: bool, committed_clear: float, now: Optional[float] = None) -> Dict[str, object]:
        ...
        return {
            ...
            "progress_age_sec": max(0.0, now - self._last_progress_time),
        }

    def _bypass_diag(self, mode: str, target_body: Tuple[float, float], now: Optional[float] = None) -> Dict[str, object]:
        now = time.time() if now is None else now
        return {
            "bypass_active": self._bypass_active,
            "bypass_side": self._bypass_side,
            "bypass_mode": mode,
            "bypass_target_body": {"x": float(target_body[0]), "y": float(target_body[1])},
            "bypass_left_sec": max(0.0, self._bypass_until - now),
            "rejoin_forced_left_sec": max(0.0, self._rejoin_forced_until - now),
        }
