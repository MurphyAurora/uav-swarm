...
ORCA_USE_WORLD_STATE=${ORCA_USE_WORLD_STATE:-1}
...
OBSTACLE_CONFIG=${OBSTACLE_CONFIG:-${WS_ROOT}/scripts/legacy/obstacles.yaml}
...
if [ "${DYNAMIC_OBS_MODE}" != "scene" ] && [ -f "${OBSTACLE_CONFIG/#\~/$HOME}" ]; then
  eval "$(python3 ${WS_ROOT}/scripts/legacy/obstacle_config.py --shell "${OBSTACLE_CONFIG}")"
fi
...
ENGINEERING_SEMI_DISTRIBUTED=${ENGINEERING_SEMI_DISTRIBUTED:-1}
LOCAL_CONTROLLER_REF_TIMEOUT=${LOCAL_CONTROLLER_REF_TIMEOUT:-0.6}
LOCAL_CONTROLLER_VIEW_TIMEOUT=${LOCAL_CONTROLLER_VIEW_TIMEOUT:-1.0}
SEMI_DISTRIBUTED_ENABLE=${SEMI_DISTRIBUTED_ENABLE:-1}
CONSENSUS_GRAPH_MODE=${CONSENSUS_GRAPH_MODE:-line}
CONSENSUS_NEIGHBOR_RADIUS=${CONSENSUS_NEIGHBOR_RADIUS:-0.0}
CONSENSUS_CORRECTION_GAIN=${CONSENSUS_CORRECTION_GAIN:-0.25}
CONSENSUS_MAX_CORRECTION=${CONSENSUS_MAX_CORRECTION:-0.35}
VIRTUAL_LAG_LIMIT=${VIRTUAL_LAG_LIMIT:-2.2}
REAR_SPEED_CONSTRAINT=${REAR_SPEED_CONSTRAINT:-1}
REAR_SAFE_GAP=${REAR_SAFE_GAP:-1.45}
REAR_LATERAL_WINDOW=${REAR_LATERAL_WINDOW:-0.9}
REAR_MAX_BRAKE=${REAR_MAX_BRAKE:-0.55}
...
export ENGINEERING_SEMI_DISTRIBUTED
export LOCAL_CONTROLLER_REF_TIMEOUT
export LOCAL_CONTROLLER_VIEW_TIMEOUT
export SEMI_DISTRIBUTED_ENABLE
export CONSENSUS_CORRECTION_GAIN
export CONSENSUS_MAX_CORRECTION
export VIRTUAL_LAG_LIMIT
export REAR_SPEED_CONSTRAINT
export REAR_SAFE_GAP
export REAR_LATERAL_WINDOW
export REAR_MAX_BRAKE
...
echo "[MODE] semi-distributed: ENGINEERING=${ENGINEERING_SEMI_DISTRIBUTED}, ENABLE=${SEMI_DISTRIBUTED_ENABLE}, GRAPH=${CONSENSUS_GRAPH_MODE}, NEIGHBOR_RADIUS=${CONSENSUS_NEIGHBOR_RADIUS}, CONSENSUS_GAIN=${CONSENSUS_CORRECTION_GAIN}, CONSENSUS_MAX_CORRECTION=${CONSENSUS_MAX_CORRECTION}, VIRTUAL_LAG_LIMIT=${VIRTUAL_LAG_LIMIT}, LOCAL_REF_TIMEOUT=${LOCAL_CONTROLLER_REF_TIMEOUT}, LOCAL_VIEW_TIMEOUT=${LOCAL_CONTROLLER_VIEW_TIMEOUT}, REAR_SPEED_CONSTRAINT=${REAR_SPEED_CONSTRAINT}, REAR_SAFE_GAP=${REAR_SAFE_GAP}, REAR_LATERAL_WINDOW=${REAR_LATERAL_WINDOW}, ORCA_USE_WORLD_STATE=${ORCA_USE_WORLD_STATE}"
...
{
  echo '[COMM_LAUNCH] id=${id} SWARM_MODE=${SWARM_MODE} AVOID_SCOPE=${AVOID_SCOPE} avoid_args=${avoid_args:-<none>}';
  ros2 run xtd2_communication multirotor_communication --model gz_x500 --id ${id} --namespace ${ns} --px4-ns ${px4_ns} --target-sys-id ${target_sys_id} ${avoid_args};
} 2>&1 | tee ${LOG_DIR}/comm_${id}.log;
exec bash
"
done
sleep 5
...
source /opt/ros/jazzy/setup.bash;
source install/setup.bash;
python3 ${WS_ROOT}/scripts/legacy/swarm_state_exchange.py --num-drones ${NUM_DRONES} --rate 15 --spawned-formation ${USE_SPAWNED_FORMATION} --mission-start-x ${MISSION_START_X} --base-y ${DYNAMIC_OBS_WALL_Y} --y-spacing ${DYNAMIC_OBS_TARGET_Y_SPACING} --leader-id ${LEADER_ID} --graph-mode ${CONSENSUS_GRAPH_MODE} --neighbor-radius ${CONSENSUS_NEIGHBOR_RADIUS} 2>&1 | tee ${LOG_DIR}/state_exchange.log;
exec bash
"

if [ "${ENGINEERING_SEMI_DISTRIBUTED}" = "1" ]; then
  echo "[INFO] start per-UAV local controllers..."
  for ((id=1; id<=NUM_DRONES; id++)); do
    launch_job "LocalCtrl_${id}" "
cd ${WS_ROOT};
source /opt/ros/jazzy/setup.bash;
source install/setup.bash;
python3 ${WS_ROOT}/scripts/legacy/uav_local_controller.py --id ${id} 2>&1 | tee ${LOG_DIR}/local_controller_${id}.log;
exec bash
"
  done
fi
...
source /opt/ros/jazzy/setup.bash;
source install/setup.bash;
python3 ${WS_ROOT}/scripts/legacy/local_avoid_orca.py --num-drones ${NUM_DRONES} --safe-radius ${ORCA_SAFE_RADIUS} --max-speed ${ORCA_MAX_SPEED} --gain ${ORCA_GAIN} --obs-gain ${ORCA_OBS_GAIN} --obs-influence-radius ${ORCA_OBS_INFLUENCE_RADIUS} --enable-after-z ${ORCA_ENABLE_AFTER_Z} --use-world-state ${ORCA_USE_WORLD_STATE} 2>&1 | tee ${LOG_DIR}/local_orca.log;
exec bash
"
