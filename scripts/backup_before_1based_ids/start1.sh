...
# 环境变量配置
PX4_SYS_ID_BASE=${PX4_SYS_ID_BASE:-1}
...
DYNAMIC_OBS_TARGET_X=${DYNAMIC_OBS_TARGET_X:-22.0}
DYNAMIC_OBS_TARGET_MARGIN_Y=${DYNAMIC_OBS_TARGET_MARGIN_Y:-1.2}
DYNAMIC_OBS_TARGET_Y_BASE=${DYNAMIC_OBS_TARGET_Y_BASE:-${DYNAMIC_OBS_WALL_Y}}
DYNAMIC_OBS_TARGET_Y_SPACING=${DYNAMIC_OBS_TARGET_Y_SPACING:-1.8}
LEADER_ID=${LEADER_ID:-1}
FORMATION_KP=${FORMATION_KP:-0.65}
LEADER_TRACK_KP=${LEADER_TRACK_KP:-0.60}
MAX_FOLLOWER_SPEED=${MAX_FOLLOWER_SPEED:-1.3}
MAX_LEADER_SPEED=${MAX_LEADER_SPEED:-1.1}
...
if [ "${SWARM_MODE}" = "hybrid" ]; then
  COMM_EXTRA_ARGS="--enable-avoid --avoid-timeout ${AVOID_TIMEOUT} --avoid-recovery-sec ${AVOID_RECOVERY_SEC} --avoid-enable-z ${COMM_AVOID_ENABLE_Z} --avoid-min-speed ${COMM_AVOID_MIN_SPEED} --avoid-z-track-tol ${COMM_AVOID_Z_TRACK_TOL}"
...
for ((i=0; i<NUM_DRONES; i++)); do
  ...
  if [ $i -eq 0 ]; then
    launch_job "PX4_$i" "
cd ~/PX4-Autopilot;
PX4_UXRCE_DDS_PORT=${DDS_PORT} PX4_GZ_WORLD=${GZ_WORLD} PX4_GZ_MODEL_POSE=\"${spawn_enu_x},${spawn_enu_y}\" PX4_SYS_AUTOSTART=4001 PX4_SIM_MODEL=gz_x500 ./build/px4_sitl_default/bin/px4 -i $((i+1)) | tee ${LOG_DIR}/px4_${i}.log;
exec bash
"
  else
    launch_job "PX4_$i" "
cd ~/PX4-Autopilot;
PX4_UXRCE_DDS_PORT=${DDS_PORT} PX4_GZ_WORLD=${GZ_WORLD} PX4_GZ_STANDALONE=1 PX4_SYS_AUTOSTART=4001 PX4_GZ_MODEL_POSE=\"${spawn_enu_x},${spawn_enu_y}\" PX4_SIM_MODEL=gz_x500 ./build/px4_sitl_default/bin/px4 -i $((i+1)) | tee ${LOG_DIR}/px4_${i}.log;
exec bash
"
  fi
  sleep 7
...
for ((id=0; id<NUM_DRONES; id++)); do
  ...
  if [ -n "${TARGET_SYS_IDS_STR}" ]; then
    target_sys_id="${SYSIDS[$id]}"
  else
    target_sys_id=$((PX4_SYS_ID_BASE + id))
  fi
  [ -z "${target_sys_id}" ] && { echo "[ERROR] 无人机${id}的target_sys_id为空"; exit 1; }
  echo "[MAP] id=${id} -> ns=${ns} -> ${px4_ns} -> target_sys_id=${target_sys_id}"

  launch_job "Comm_${id}" "
cd ~/ws_xtd2;
source /opt/ros/jazzy/setup.bash;
source install/setup.bash;
ros2 run xtd2_communication multirotor_communication --model gz_x500 --id ${id} --namespace ${ns} --px4-ns ${px4_ns} --target-sys-id ${target_sys_id} ${COMM_EXTRA_ARGS} 2>&1 | tee ${LOG_DIR}/comm_${id}.log;
exec bash
"
done
sleep 5
...
source install/setup.bash;
  ...
  python3 ~/ws_xtd2/scripts/dynamic_obstacle_source.py --rate 10 --world ${DYNAMIC_OBS_WORLD} --visualize-gz ${DYNAMIC_OBS_VISUALIZE_GZ} --mode static_wall --wall-x ${DYNAMIC_OBS_WALL_X} --wall-y ${DYNAMIC_OBS_WALL_Y} --wall-z ${DYNAMIC_OBS_WALL_Z} --wall-length ${DYNAMIC_OBS_WALL_LENGTH} --wall-thickness ${DYNAMIC_OBS_WALL_THICKNESS} --wall-height ${DYNAMIC_OBS_WALL_HEIGHT} --wall-segment-spacing ${DYNAMIC_OBS_WALL_SEGMENT_SPACING} --target-ball-enable 1 --target-ball-num-drones ${NUM_DRONES} --target-ball-leader-id ${LEADER_ID} --target-ball-target-x ${DYNAMIC_OBS_TARGET_X} --target-ball-base-y ${DYNAMIC_OBS_TARGET_Y_BASE} --target-ball-target-z ${MISSION_Z} --target-ball-y-spacing ${DYNAMIC_OBS_TARGET_Y_SPACING} 2>&1 | tee ${LOG_DIR}/dynamic_obstacles.log;
exec bash
"
  fi
fi
...
for ((id=0; id<NUM_DRONES; id++)); do
  ns="x500_${id}"
  y_offset=$(awk "BEGIN{printf \"%.2f\", ${DYNAMIC_OBS_WALL_Y} + ((${id}-${LEADER_ID})*${DYNAMIC_OBS_TARGET_Y_SPACING})}")
...
source install/setup.bash;
# 单行紧凑YAML格式，彻底解决解析错误
  timeout ${WARMUP_RUN_SEC}s ros2 topic pub -r 20 /xtdrone2/${ns}/cmd_pose_local_ned geometry_msgs/msg/Pose \"{position: {x: 0.0, y: ${y_offset}, z: ${TAKEOFF_Z}}, orientation: {x: 0.0, y: 0.0, z: 0.0, w: 1.0}}\" > "${LOG_DIR}/warmup_${id}.log" 2>&1;
exec bash
"
done
...
) | tee ${LOG_DIR}/control_cluster.log
EOF
)
