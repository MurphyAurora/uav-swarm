...
The framework is intentionally ROS-independent at this layer.  ROS2 nodes can
convert messages into these dataclasses, while the planning logic can be tested
as plain Python before being connected to PX4/Gazebo.
"""
...
class Vec3:
    """Small 3-D vector helper using the same numeric fields as NED commands."""
    ...
    @staticmethod
    def from_mapping(data: Dict, default: Optional["Vec3"] = None) -> "Vec3":
        default = default or Vec3()
        return Vec3(
            float(data.get("x", data.get("world_x", default.x))),
            float(data.get("y", data.get("world_y", default.y))),
...
@dataclass
class PlannerState:
    """Current UAV state in the planner frame.

    The code does not assume ENU or NED internally; it only requires that all
    positions, velocities, and obstacles are expressed in the same frame before
    entering the planner core.  command_output.py is responsible for final
    command conventions.
    """
    ...
    @staticmethod
    def from_mapping(data: Dict) -> "PlannerState":
        return PlannerState(
            drone_id=int(data.get("id", data.get("drone_id", 0))),
            position=Vec3.from_mapping(data),
            velocity=Vec3(
...
@dataclass
class Obstacle:
    """Static or dynamic obstacle used by the planner safety checker."""

    position: Vec3
    radius: float = 0.3
    velocity: Vec3 = field(default_factory=Vec3)
    source: str = "obstacle"  # static, dynamic, drone, lidar_near_field
    obstacle_id: str = ""
    predictions: List[Tuple[float, Vec3]] = field(default_factory=list)

    def position_at(self, t: float) -> Vec3:
        """Return predicted obstacle position at relative time t."""
        if self.predictions:
...
class CandidateTrajectory:
    """Candidate local trajectory or constant-velocity primitive."""
...
class PlannerCommand:
    velocity: Vec3
    mode: str = "planner"  # planner, fallback_hover, fallback_back, fallback_climb
...
@dataclass
class PlannerConfig:
    """Planner parameters with conservative defaults for incremental rollout."""
...
def nearest_prediction(predictions: Iterable[Tuple[float, Vec3]], t: float) -> Vec3:
    """Return the predicted position with timestamp closest to t."""
    best_t = None
    best_pos = None
    for pred_t, pred_pos in predictions:
        err = abs(float(pred_t) - float(t))
        if best_t is None or err < best_t:
            best_t = err
            best_pos = pred_pos
